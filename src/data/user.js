import { faFigma, faGit, faGitAlt, faJs, faNode, faReact, faSwift } from "@fortawesome/free-brands-svg-icons";

const INFO = {
	main: {
		title: "Portfolio by Uluc Hosgelen",
		name: "Uluc Hosgelen",
		email: "uluchosgelen@gmail.com",
		logo: "uluc-logo3.png",
	},

	socials: {
		twitter: "https://twitter.com/uluc130/",
		github: "https://github.com/uluc130/",
		linkedin: "https://www.linkedin.com/in/uluc-hosgelen-6894761b/",
		instagram: "https://instagram.com/uluc130/",
		stackoverflow: "https://stackoverflow.com/",
		facebook: "https://facebook.com/"
	},

	homepage: {
		title: "Hi, I’m Uluc Hosgelen",
		subtitle: "iOS developer, and triathlete.",
		description:
			"I’m an aspiring iOS developer with a passion for sports, especially triathlons. I am also interested in backend development, databases, and working with APIs. In my free time, I enjoy training for triathlons and exploring new routes. I am always looking for new challenges and opportunities to grow as a developer.",
	},

	about: {
		title: "I’m Uluç Hoşgelen. I live in Istanbul.",
		description:"I am a software developer with a passion for challenging myself. After a successful and long career in a different field, I decided to pursue my dream of becoming a software developer. ",
		current:"I have recently been learning iOS development and have focused developing mobile applications using native Swift language and Apple’s modern framework SwiftUI. I am excited to apply my skills and knowledge to create innovative and user-friendly mobile applications with clean code and good design principles.",
		interest: "I like to deal with data, interested in databases, api’s and backend development. My learning process will never end. Every day I am adding new skills to my skillset, with my goal in my mind to become a full stack developer.",
	},

	articles: {
		title: "I'm passionate about pushing the boundaries of what's possible and inspiring the next generation of innovators.",
		description:
			"Chronological collection of my long-form thoughts on programming, leadership, product design, and more.",
	},

	projects: [
		{
			title: "Twitter Api",
			description: "Local RESTApi Server created for Twitter-Clone course project with Node.js, Express, Mongoose & local MongoDB database",
			logo: "https://cdn.jsdelivr.net/npm/programming-languages-logos/src/javascript/javascript.png",
			logo2: "",
			linkText: "View Project",
			link: "https://github.com/uluc130/twitter-api",
		},

		{
			title: "Twitter Clone",
			description: "Front-End created using SwiftUI for FullStack Twitter-Clone app project course",
			logo: "https://cdn.jsdelivr.net/npm/programming-languages-logos/src/swift/swift.png",
			linkText: "View Project",
			link: "https://github.com/uluc130/twitter",
		},

		{
			title: "Little Lemon Restaurant",
			description:
				"Capstone project for Coursera Meta IOS Developer program.",
			logo: "https://cdn.jsdelivr.net/npm/programming-languages-logos/src/swift/swift.png",
			linkText: "View Project",
			link: "https://github.com/uluc130/littlelemon",
		},

		
	],
	eduDetails: [
		{
		  Position: "Frontend Development",
		  Company: "Udemy, YouTube, Google, Medium",
		  Location: "Online",
		  Type: "Full Time",
		  Duration: "Jan 2022 - Present",
		},
		{
		  Position: "Bachelor in Electronics & Communication",
		  Company: `Your College Name here`,
		  Location: "Bengaluru",
		  Type: "Full Time",
		  Duration: "Aug 2020 - Present",
		},
	  ],
	certifications: [
		{
			name: "Meta iOS Developer Specializations",
			teacher: "Meta",
			issuedBy: "Coursera",
			date:"September 2023",
			link: "https://www.coursera.org/account/accomplishments/specialization/certificate/HHKWQZDP3MTM" 
		},
		{
			name: "SwiftUI & Node.JS FullStack",
			teacher: "Cem Salta",
			issuedBy: "Udemy",
			date:"October 2023",
			link: "https://www.udemy.com/certificate/UC-bd988e18-52d3-4432-81eb-3489bf2cde20/" 
		}
	],
	techStack: [
		{
			name: "Swift",
			logo: "ci-2xl ci-swift",
			type: "language"
		},
		{
		name: "JavaScript",
		logo: "ci-2xl ci-js",
		type: "language"
	},
	{
		name: "Node.Js",
		logo: "ci-2xl ci-nodejs",
		type: "language"
	},
	{
		name: "React",
		logo: "ci-2xl ci-reactjs",
		type: "language"
	},
	{
		name: "React Native",
		logo: "ci-2xl ci-react-native",
		type: "language"
	},
	{
		name: "Git",
		logo: "ci-2xl ci-git",
		type: "language"
	}, 
	{
		name: "Figma",
		logo: "ci-2xl ci-figma",
		type: "language"
	},
	{
		name: "Firebase",
		logo: "ci-2xl ci-firebase",
		type: "Saas",
	}
	,
	{
		name: "MongoDb",
		logo: "ci-2xl ci-mongodb2",
		type: "Saas"
	},
	{
		name: "Css",
		logo: "ci-2xl ci-css",
		type: "Saas"
	},

]
};


  
export default INFO
